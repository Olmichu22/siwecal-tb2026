#
# Gaudi/k4FWCore steering for TB2026CERN_eudaq_run_000210 (th210): the VALIDATION / EDM4hep stage.
# Reads the event-built `ecal` tree produced by steer_TB2026CERN_eudaq_run_000210.py and writes the
# EDM4hep + PID output next to it, in <run>/events/ -- NOT in the shared
# Reconstruction/ area.
#
# Run steer_TB2026CERN_eudaq_run_000210.py first (it makes <run>/events/ecal_<run>.root).
#
...
#   k4run gaudi_jobs/angular_scan/validate_TB2026CERN_eudaq_run_000210.py
#
import os
...
_RUN = "TB2026CERN_eudaq_run_000210"
...
if not _f or _f.IsZombie():
    raise SystemExit(f"input not found (run steer_TB2026CERN_eudaq_run_000210.py first): {_INPUT}")
