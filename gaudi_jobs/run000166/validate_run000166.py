#
# Gaudi/k4FWCore steering for TB2026CERN_eudaq_run_000166 (th210): the VALIDATION
# / EDM4hep stage. Reads the event-built `ecal` tree produced by
# steer_run000166.py and writes the EDM4hep + PID output next to it, in
# <run>/events/ -- NOT in the shared Reconstruction/ area.
#
# Run steer_run000166.py first (it makes <run>/events/ecal_<run>.root).
#
...
#   k4run gaudi_jobs/run000166/validate_run000166.py
#
import os
...
if not _f or _f.IsZombie():
    raise SystemExit(f"input not found (run steer_run000166.py first): {_INPUT}")
