...
h_q.Draw("hist")
h_b.Draw("hist same")
leg = ROOT.TLegend(0.36, 0.15, 0.76, 0.31)
leg.SetBorderSize(0)
leg.SetFillStyle(0)
...
g_cb.Draw("plsame")
g_db.Draw("plsame")
leg2 = ROOT.TLegend(0.32, 0.15, 0.88, 0.32)
leg2.SetBorderSize(0)
leg2.SetFillStyle(0)
leg2.SetTextSize(0.030)
leg2.AddEntry(g_cb, "binned by the CHIP's own hits (climbs to 53%)", "pl")
leg2.AddEntry(g_db, "binned by the whole DETECTOR's hits (saturates ~32%)", "pl")
leg2.Draw()
keep += [fr2, g_cb, g_db, leg2]
...
tgt.SetLineWidth(2)
tgt.Draw()
leg4 = ROOT.TLegend(0.16, 0.71, 0.66, 0.89)
leg4.SetBorderSize(0)
leg4.SetFillStyle(0)
leg4.SetTextSize(0.030)
leg4.AddEntry(h_r, "as calibrated", "l")
leg4.AddEntry(h_m, f"minus per-slice common mode (RMS -{cm_rms:.1f}%)", "l")
leg4.AddEntry(tgt, f"out-of-spill pedestal RMS ({OUT_RMS:.2f} ADC)", "l")
leg4.AddEntry(0, f"occupancy-offset instead: RMS -{gain_rms:.1f}% (nothing)", "")
leg4.Draw()
keep += [h_r, h_m, tgt, leg4]
