...
GAUDI_CALIB_DIR = os.path.join(paths.calib_dir(), "MuonCalib_gaudi")
...
def resolve_gaudi_calib_files(th):
    """Return (ped_hg, mip_hg, ped_lg, mip_lg) from ``MuonCalib_gaudi`` for threshold ``th``.

    Same role as :func:`resolve_muon_calib_files`, but for the tables generated by
    the Gaudi ``PedestalMipCalibrator`` -- which are named after the calibration
    run they came from (``*_run_000004_highgain.txt``), not after the threshold,
    so the files are globbed rather than spelled out.

    A missing ``th{th}`` folder, or one holding no high-gain table, is a hard
    error: silently falling back to another threshold's tables is exactly how a
    run ends up reconstructed against the wrong pedestals.

    The LOW-gain pair is optional and comes back as ``("", "")`` when absent --
    the event builder then keeps taking every hit's energy from the high gain,
    ...
    """
    def _one(kind, pattern, required):
        folder = os.path.join(GAUDI_CALIB_DIR, kind, f"th{th}")
        if not os.path.isdir(folder):
            if not required:
                return ""
            print(f"ERROR: calibration folder not available for threshold {th}: {folder}",
                  file=sys.stderr)
            sys.exit(1)
        # Run numbers are zero-padded, so lexicographic sort = numeric order.
        candidates = sorted(glob.glob(os.path.join(folder, pattern)))
        if not candidates:
            if not required:
                return ""
            print(f"ERROR: calibration folder for threshold {th} holds no {kind} "
                  f"table matching {pattern}: {folder}", file=sys.stderr)
            sys.exit(1)
        return candidates[-1]

    ped_hg = _one("pedestals", "Pedestal_*_highgain.txt", required=True)
    mip_hg = _one("mips", "MIP_pedestalsubmode1_*_highgain.txt", required=True)
    ped_lg = _one("pedestals", "Pedestal_*_lowgain.txt", required=False)
    mip_lg = _one("mips", "MIP_pedestalsubmode1_*_lowgain.txt", required=False)
