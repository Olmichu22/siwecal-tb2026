...
def resolve_gaudi_calib_files(th):
    """Return (pedestal_path, mip_path) from ``MuonCalib_gaudi`` for threshold ``th``.
    ...
    so both files are globbed rather than spelled out.
    ...
    """
    resolved = []
    for kind, pattern in (("pedestals", "Pedestal_*_highgain.txt"),
                          ("mips", "MIP_pedestalsubmode1_*_highgain.txt")):
        ...
        if not candidates:
            print(f"ERROR: calibration folder for threshold {th} holds no {kind} "
                  f"high-gain table: {os.path.join(folder, pattern)}", file=sys.stderr)
            sys.exit(1)
